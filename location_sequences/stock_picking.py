...
class stock_location(osv.osv):
    ...
    #       'group_ids': fields.many2many('res.groups', 'stock_journal_rel', 'sequence_id', 'groups_id', 'Groups'),
            'despacho_auto': fields.boolean('Despacho automatico',help="Permite hacer despacho automatico cuando de hace una venta desde Pedido de Venta"), #PERMITE A LAS TIENDAS VENDER Y QUE HAGAN DESPACHO AUTOMATICO
    }
...
class stock_picking(osv.osv):
    ...
    def do_partial(self, cr, uid, ids, partial_datas, context=None): # PARA ENTRGAS PARCIALES
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            for move in too_few:
                ...
                if not new_picking and empty_picking:
    ...
    #PARA QUE CUANDO SE GENERE UNA NOTA DE CREDITO DESDE LA GUIA DE SALIDA ESTE COJA EL PARTNER PADRE DEL CONTACTO.
    def _get_partner_to_invoice(self, cr, uid, picking, context=None):
        return picking.partner_id and picking.partner_id.parent_id.id or picking.partner_id.id
    ...
    def check_assign(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if move.state in ('confirmed', 'waiting'):
                ...
                #NO PERMITE VERDER SI EN LAS TIENDAS NO HAY STOCK DE UN PRODUCTO DETERMINADO
                else:
                    '''
                    if not move.picking_id.location_id.despacho_auto and move.location_dest_id.usage == 'transit':
                        continue
                    else:
    ...
    def _update_average_price(self, cr, uid, move, context=None):
        ...
        if (move.picking_id.type == 'in') and (move.product_id.cost_method == 'average'):
            ...
            if qty > 0:
                ...
                #COMENTADO PARA EVITAR QUE SE PROMEDIO EL COSTO AUTOMATICAMENTE
...
class sale_order(osv.osv):
    ...
    def _prepare_order_picking(self, cr, uid, order, context=None):
        ...
        return {
            ...
            'auto_picking': True if (order.shop_id.warehouse_id.lot_stock_id.despacho_auto) else False, #DESPACHO AUTOMATICO
    ...
    def _create_pickings_and_procurements(self, cr, uid, order, order_lines, picking_id=False, context=None):
        ...
        if picking_id:
            ...
            #AGREGADO PARA DESPACHO AUTOMATICO
            if order.shop_id.warehouse_id.lot_stock_id.despacho_auto:
                picking_obj = self.pool.get('stock.picking') #AGREGADO PARA PROCESO INMEDIATO
                picking_obj.action_assign(cr, uid, [picking_id])
                picking_obj.force_assign(cr, uid, [picking_id]) #AGREGADO PARA PROCESO INMEDIATO
            #COMENTADO POR QUE CAUA CONFUSION
            '''
            if not order.shop_id.warehouse_id.lot_stock_id.despacho_auto: #PARA PEDIDOS DE ALMACEN PRINCIPAL
                picking_obj.action_assign(cr, uid, [picking_id], context) #PARA CUANDO DE EJECUTE EL PICKING Y PASE A ESTADO RESERVADO AUTOMATICAMENTE
            '''
                #_logger.error("stcockreeee----: %r", m)
...
class purchase_order(osv.osv):
    ...
    def _prepare_order_line_move(self, cr, uid, order, order_line, picking_id, context=None):
        if context is None:
            context = {}    
        mov_lines =  super(purchase_order, self)._prepare_order_line_move(cr, uid, order, order_line, picking_id, context)
        ...
        mov_lines.update({
                'price_unit': price_unit, #agregado nuevos cambios
            })
