            ...
            for line in order.lines:
                if order.state == 'draft':
                    if order.almacendespacho_id:
                        ...
                        _logger.error("PINCKING 1: %r", line.product_id.qty_disponible)
                        if line.product_id.qty_disponible < line.qty:
...
class pos_order_line(osv.osv):
    ...
    def onchange_product_id(self, cr, uid, ids, pricelist, product_id, qty=0, partner_id=False, almacendespacho=False, session_id=False, context=None):
        ...
        if almacendespacho:
            ...
            #_logger.error("LOOOOOOOOOOOO: %r", session.config_id.shop_id.warehouse_id.lot_stock_id.id)
        ...
        if almacendespacho:
            ...
            if prod_id.qty_disponible < qty:
                raise osv.except_osv(_('Warning!'),_('No puede vender un producto con stock insuficiente, Producto: "%s" (Cant. Dispo:%d).') %(prod_id.name, prod_id.qty_disponible,))                
        #FIN        
    ...
    def onchange_qty(self, cr, uid, ids, product, discount, qty, price_unit, almacendespacho=False, context=None):
        ...
        if almacendesp:
            ...
            if prod.qty_disponible < qty:
                raise osv.except_osv(_('Warning!'),_('No puede vender un producto con stock insuficiente, Producto: "%s" (Cant. Dispo:%d).') %(prod.name, prod.qty_disponible,))
        '''    
        #FIN
