...
class purchase_landing_cost_position(osv.osv):
    ...
    _columns = {
        ...
        'amount': fields.float('Amount', required=True, digits_compute=dp.get_precision('PrecioCosto'), help="""Landing cost for stock valuation. It will be added to the price of the supplier price."""),
        ...
        'distribution_type': fields.selection( [('per_value','Per Value')], 'Amount Type', required=True, help="Defines if the amount is to be calculated for each quantity or an absolute value"),
        ...
        'is_invoiced': fields.boolean('Generar Factura'),
        'tipo_doc': fields.selection([('F','Factura'),('B','Boleta'),('R','recibo'),('D','DUA')],'Tipo Doc'),
        'num_doc': fields.text('Nro Fact', size=64,),
        'fecha': fields.date('Fecha Emision'),
        'tipo_cambio': fields.float('Tipo de cambio',digits=(2,3)),
        'currency_id': fields.many2one('res.currency', 'Moneda',),
        'igv': fields.float('IGV', required=True, digits_compute=dp.get_precision('PrecioCosto')),
