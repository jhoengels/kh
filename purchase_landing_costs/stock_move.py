...
class stock_move(osv.osv):
    ...
    def _landing_costs_price_unit_with_costs(self, cr, uid, ids, name, args, context):
        ...
        for line in self.browse(cr, uid, ids):
            if line.price_unit_without_costs and line.price_unit_without_costs <> 0.0:                
                price_unit_with_costs = line.price_unit_without_costs*line.tipo_cambio
                #_logger.error("OOOorig1----: %r", price_unit_with_costs)

            else:
                price_unit_with_costs = line.price_unit*line.tipo_cambio or 0.0
            ...
            if line.product_id.landing_cost_calculate:
                if line.picking_id and (line.picking_id.landing_costs_per_value > 0.0 or line.picking_id.landing_costs_per_unit > 0.0):       
                    ...
                    if line.picking_id.landing_costs_base_amount > 0.0 and line.product_qty > 0.0:
                        price_unit_with_costs += ((line.picking_id.landing_costs_per_value / line.picking_id.landing_costs_base_amount) * (line.price_unit*line.tipo_cambio* line.product_qty)) / line.product_qty
                        #_logger.error("OOOori2----: %r", ((line.picking_id.landing_costs_per_value / line.picking_id.landing_costs_base_amount) * (line.price_unit*line.tipo_cambio*line.product_qty)) / line.product_qty)
    ...
    _columns = { 
          'price_unit_without_costs': fields.float('', digits_compute=dp.get_precision('PrecioCosto'), ),
          'price_unit_with_costs': fields.function(_landing_costs_price_unit_with_costs, digits_compute=dp.get_precision('PrecioCosto'), ),
          ...
          'landing_costs_base_quantity': fields.function(_landing_costs_base_quantity, digits_compute=dp.get_precision('PrecioCosto'), string='Stock Move Quantity For Per Unit Calculation'),
          'landing_costs_base_amount': fields.function(_landing_costs_base_amount, digits_compute=dp.get_precision('PrecioCosto'), string='Stock Move For Per Value Calculation'),
