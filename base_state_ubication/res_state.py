...
class res_state(osv.osv):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        for state in self.browse(cr, uid, ids, context=context):
            ...
            while acc:
                ...
                acc = acc.parent_id
            data = ' / '.join(data)
            #res.append((state.id, (state.code and '[' + state.code + '] ' or '') + data)) #Se comento para que no salga el codigo
    ...
    def _name_get_fnc_normal(self, cr, uid, ids, prop, unknow_none, context=None):
        if not ids:
            return []
        res = []
        for state in self.browse(cr, uid, ids, context=context):
            ...
            acc = state
            while acc:
                data.append(acc.name)
                #_logger.error("DISTRITOO----: %r", data)
                acc = acc.parent_id
            data = ' / '.join(data)           
            res.append((state.id, data))
        return dict(res)
    ...
    _columns = {
            ...
            'complete_name_normal': fields.function(_name_get_fnc_normal, method=True, type="char", string='Complete Name', fnct_search=complete_name_search),
